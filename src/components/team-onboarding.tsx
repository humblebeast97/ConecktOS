...
export function TeamOnboarding({ compact = false }: { compact?: boolean }) {
  ...
  // The invite just created, highlighted in the Invite links panel.
  ...
  // sets the terms here (role + commission) and shares an invite link. The mock
  ...
  const submit = () => {
    if (mode === "supabase") {
      ...
      // Staff sign in with their own account, so we generate an invite the owner
      // shares as a link (/join?code=...) from the Invite links panel.
      guarded(async () => {
        try {
          ...
          toast.success("Invite link ready", {
            description: `Share the ${industryRoleLabel(form.role)} link from Invite links below.`,
          });
          setForm(emptyForm);
          setStep(0);
        } catch (e) {
  ...
  return (
    ...
    >
      <div className="h-fit space-y-5">
          ...
          <div className="mt-5 min-h-[13rem] space-y-4">
            {inviteMode ? (
              <p className="rounded-xl border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
                Set the role and commission, then share the invite link. Your teammate opens it,
                creates their account and enters their own name, job title and payout details.
              </p>
            ) : null}
            {!inviteMode && step === 0 ? (
              ...
              {step >= lastStep ? (
                isSubmitting ? (
                  ...
                  <>
                    ...
                    {inviteMode ? "Create invite link" : "Add to team"}
                  </>
                )
              ) : (
                <>
        ...
        {inviteMode ? <InvitesPanel highlightCode={lastInviteCode} /> : null}
      </div>
